...
class SegmentationPreprocessing:
    ...
    def save_image(self) -> None:
        ...
        try:
            object_from_s3 = s3.get_object(Bucket=SOURCES_BUCKET, Key=self.img_name)
    ...
    @staticmethod
    def mask_to_image(mask) -> PIL.Image.Image:
        mask = cv2.resize(mask, (854, 480), interpolation=cv2.INTER_NEAREST)
        mask = mask.reshape(480, 854, 1).astype(np.float32)

        return mask

    def masks_to_one_mask(self, masks: Dict[str, PIL.Image.Image]) -> PIL.Image.Image:
        masks_colors = {
